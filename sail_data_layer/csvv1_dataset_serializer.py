...
class Csvv1DatasetSerializer(BaseDatasetSerializer):
    ...
    def read_dataset_for_path(self, path_dir_dataset_source: str) -> any:
        ...
        with ZipFile(path_file_data_content_zip) as archive_data_content:
            ...
            for name_file in archive_data_content.namelist():
                ...
                if data_frame_name == "static_data_table":
        ...
        with ZipFile(path_file_data_content_zip) as archive_data_content:
            for name_file in archive_data_content.namelist():
                ...
                for name_series in data_model_data_frame.list_series_name:
                    if (
                        data_model_data_frame[name_series].data_type
    ...
    def write_dataset(self, tabular_dataset: TabularDataset) -> None:
        self.write_dataset_for_path(
            os.path.join(self.path_dir_dataset_store, tabular_dataset.dataset_id),
            tabular_dataset,
        )

    def write_dataset_for_path(
        self, path_dir_dataset_target: str, tabular_dataset: TabularDataset
    ) -> None:
        if os.path.isdir(path_dir_dataset_target):
            shutil.rmtree(path_dir_dataset_target)
        os.makedirs(path_dir_dataset_target)
        path_file_dataset_header = os.path.join(
            path_dir_dataset_target, "dataset_header.json"
        )
        path_file_data_model = os.path.join(path_dir_dataset_target, "data_model.zip")
        path_file_data_content = os.path.join(
            path_dir_dataset_target, "data_content.zip"
        )
        ...
        # write data model
        with ZipFile(
            path_file_data_model, "w", ZIP_DEFLATED, compresslevel=9
        ) as zip_file_data_model:
            name_file = "data_model.json"
            zip_file_data_model.writestr(
                name_file, json.dumps(tabular_dataset.data_model.to_dict())
            )

        # write data content
        with ZipFile(
            path_file_data_content, "w", ZIP_DEFLATED, compresslevel=9
        ) as zip_file_data_content:
